...
Stockfish1 = chess.engine.SimpleEngine.popen_uci(stockfish_faster_path)
Stockfish2 = chess.engine.SimpleEngine.popen_uci(stockfish_moreCompatible_path)
...
with open('dati.csv', mode='a', newline='') as file:
    ...
    for i in range(num_games):
        result = play_game(Stockfish1, Stockfish2)
        results[result[0]] += 1
        print(f"Game {i+1}: {result[0]}")
        if (result[0] == "1-0") :
